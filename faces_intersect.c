...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	for( int f=0; f < nFaces[0]; f++){
		...
		if( t_current < 0.0001 ||  t_current>rl[0]) continue;
		
		if(t_current < t[0]){ // save closest
		 	t[0] = t_current;
		 	faceI[0] = f+1;
		 	//mexPrintf("mex - intersect. face %d,  %f \n",  f+1, t[0]);
		 	//mexPrintf("mex - edge1 (%.2f,%.2f,%.2f)\n",  edge1[0],edge1[1],edge1[2]);
		 	//mexPrintf("mex - edge2 (%.2f,%.2f,%.2f)\n\n",  edge2[0],edge2[1],edge2[2]);
		}
		//break;
	}
}